...
class PagesController extends Controller
{
    ...
    public function getDistrict($vill_provinceid=0){
      ...
      $userData=Page::getDistrict($vill_provinceid); 
    ...
    public function getTumbol($vill_districtid=0){
     ...
     public function getVillage($vill_districtid=0,$vill_tumbolid=0){
      ...
      header('Access-Control-Allow-Headers: *');
      // Fetch Employees by Departmentid
      $userVill['data'] = Page::gettumbolVillage($vill_districtid,$vill_tumbolid);
    ...
    public function editblockage($uid){
      ...
      $user = Blockage::where('blk_id', $uid)->value('blk_user_id');
      $userId = auth()->user()->id;
      ...
      //  $json = file_get_contents('http://127.0.0.1:8000/getBlockageID/'.$uid);
      $obj = $data;
      ...
      // dd($obj[0]->blockageLocation->blk_district);
      // Load index view
      // dd($ProblemDetail[0]->nat_weed_detail);
      $location = BlockageLocation::where('blk_location_id', $obj[0]->blockageLocation->blk_location_id)->get();
      // dd($location[0]->blk_start_utm->getLat());
